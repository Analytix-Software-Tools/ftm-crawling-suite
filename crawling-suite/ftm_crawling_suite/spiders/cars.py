...
class CarsSpider(CrawlSpider):
    ...
    start_urls = [
        'https://www.cars.com/shopping/results/?stock_type=cpo&makes%5B%5D=&models%5B%5D=&list_price_max=&page_size=100&maximum_distance=all&zip=']
    custom_settings = {
        'ITEM_PIPELINES': {
    ...
    def parse_vehicle_details(self, response):
        ...
        activity_search_results = json.loads(
            response.css('script:contains("initialActivity")::text').re_first(initial_activity_regex))
        detail_search_results = json.loads(
            response.css('script:contains("dispatcher")::text').re_first(dispatcher_regex))
        item = RawDataRef()
        ...
        item['uniqueId'] = activity_search_results["vin"]
        yield item
    ...
    def parse_item(self, response):
        ...
        try:
            ...
            for i in range(0, len(search_results['vehicleArray'])):
                yield Request(
                    url=f"https://www.cars.com/vehicledetail/{search_results['vehicleArray'][i]['listing_id']}/",
